...
export function assembleReport(input: ReportAssemblyInput): any {
  ...
  return {
    ...
    // FIX: These 3 fields were generated by Agent 2A but silently dropped from the DB-stored report.
    // Every existing scan was missing moat_narrative, urgency_horizon, and threat_timeline in the DB
    // even though the UI was trying to render them. Adding them here fixes all future scans.
    moat_narrative: validatedAgent2?.moat_narrative || null,
    urgency_horizon: validatedAgent2?.urgency_horizon || null,
    threat_timeline: validatedAgent2?.threat_timeline || null,
