...
export default function Card5JobsTracker({ cardData, onBack, onNext, analysisId }: { cardData: any; onBack: () => void; onNext: () => void; analysisId?: string | null }) {
  ...
  return (
    <CardShell>
      ...
      />
      <CardBody>
        {(d?.fear_hook || d?.tough_love || d?.hope_bridge || d?.confrontation) && (
          <div style={{ marginBottom: 22, display: "grid", gap: 12 }}>
            ...
            {d?.confrontation && (
              <div style={{ background: "var(--mb-navy-tint)", borderLeft: "3px solid var(--mb-navy)", borderRadius: 8, padding: "12px 14px" }}>
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14.5, fontWeight: 800, color: "var(--mb-ink)", lineHeight: 1.6, margin: 0 }}>→ {d.confrontation}</p>
              </div>
            )}
          </div>
        )}
        ...
        {jobs.map((job: any, i: number) => {
          ...
          // ── Grounding gate ──
          // Only deterministic Naukri listings (`source === "apify_naukri"`) have a
          // match_pct computed from real shared-skill intersection (see buildWhyFit
          // / toMatchPct in apify-naukri-jobs/index.ts). Everything else is LLM
          // prose against a 220-char snippet — plausible but unverified. Strip
          // the spurious % number and label rationale clearly.
          const isGroundedListing = Boolean(job.verified_live) || job.source === "apify_naukri";
          return (
            <div key={`${job.company || "company"}-${job.title || job.role || i}`} style={{ background: "white", border: job.weak_signal ? "1.5px dashed var(--mb-rule)" : "1.5px solid var(--mb-rule)", borderRadius: 16, padding: 20, marginBottom: 14, boxShadow: "0 2px 12px rgba(0,0,0,0.04)", opacity: job.weak_signal ? 0.78 : 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-start", marginBottom: 8, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
                  {isGroundedListing ? (
                    ...
                    <span title="Match estimate generated by AI from a short snippet — not computed from full job description" style={{ fontSize: 11, fontWeight: 800, padding: "5px 10px", borderRadius: 999, background: "var(--mb-paper)", color: "var(--mb-ink2)", border: "1px dashed var(--mb-rule)", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: "0.04em" }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
                ...
                {live && !isGenericLink && isGroundedListing && <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, fontWeight: 800, color: "var(--mb-green)", background: "var(--mb-green-tint)", border: "1px solid rgba(26,107,60,0.2)", borderRadius: 999, padding: "5px 10px" }}>Verified live</span>}
                {!isGroundedListing && <span title="Listing summary written by AI from a short search-result snippet" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, fontWeight: 800, color: "var(--mb-ink2)", background: "var(--mb-paper)", border: "1px dashed var(--mb-rule)", borderRadius: 999, padding: "5px 10px" }}>AI-summarised</span>}
              ...
              {job.why_fit && (
                isGroundedListing ? (
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, lineHeight: 1.7, color: "var(--mb-ink2)", marginBottom: 10 }}>{job.why_fit}</div>
                ) : (
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, lineHeight: 1.7, color: "var(--mb-ink2)", marginBottom: 10, fontStyle: "italic", borderLeft: "2px solid var(--mb-rule)", paddingLeft: 10 }}>
                    <span style={{ fontStyle: "normal", fontWeight: 700, color: "var(--mb-amber)", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.05em", display: "block", marginBottom: 3 }}>AI summary · not verified against listing text</span>
                    {job.why_fit}
                  </div>
                )
              )}
