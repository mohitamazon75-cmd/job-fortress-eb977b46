...
export default function ResumeWeaponizerCard({ report, scanId }: { report: ScanReport; scanId?: string }) {
  ...
  const fetchWeaponized = async (angle?: string) => {
    ...
    try {
      const { data: result, error: err } = await supabase.functions.invoke('resume-weaponizer', {
        body: { report, scanId, targetRole: targetRole || undefined, jdText: jdText.trim() || undefined, angle: angle || undefined },
      });
  ...
  // Build a plain-text resume bundle for download
  const downloadResumeBundle = useCallback(() => {
    if (!data) return;
    const lines: string[] = [];
    lines.push('═══════════════════════════════════════════');
    lines.push('  WEAPONIZED RESUME — Generated by JobBachao');
    lines.push('═══════════════════════════════════════════\n');
    if (data.linkedin_headline) { lines.push('── LINKEDIN HEADLINE ──'); lines.push(data.linkedin_headline + '\n'); }
    if (data.professional_summary) { lines.push('── PROFESSIONAL SUMMARY ──'); lines.push(data.professional_summary + '\n'); }
    if (data.key_skills_section?.headline_skills?.length) { lines.push('── KEY SKILLS (ADD THESE) ──'); lines.push(data.key_skills_section.headline_skills.join(' · ') + '\n'); }
    if (data.key_skills_section?.strategic_keywords?.length) { lines.push('── STRATEGIC KEYWORDS ──'); lines.push(data.key_skills_section.strategic_keywords.join(' · ') + '\n'); }
    if (data.key_skills_section?.skills_to_remove?.length) { lines.push('── SKILLS TO REMOVE / REPLACE ──'); data.key_skills_section.skills_to_remove.forEach(s => lines.push('  ✗ ' + s)); lines.push(''); }
    if (data.experience_bullets?.length) {
      lines.push('── EXPERIENCE BULLETS (STAR FORMAT) ──');
      data.experience_bullets.forEach((b, i) => { lines.push(`\n${i + 1}. ${b.weaponized_bullet}`); if (b.why_better) lines.push(`   Why: ${b.why_better}`); });
      lines.push('');
    }
    if (data.ats_optimization?.format_tips?.length) { lines.push('── ATS FORMAT TIPS ──'); data.ats_optimization.format_tips.forEach(t => lines.push('  • ' + t)); lines.push(''); }
    if (data.cover_letter_hook) { lines.push('── COVER LETTER OPENING HOOK ──'); lines.push(data.cover_letter_hook + '\n'); }
    if (data.jd_match_analysis?.missing_keywords?.length) { lines.push('── JD KEYWORDS YOU MUST STILL ADD MANUALLY ──'); data.jd_match_analysis.missing_keywords.forEach(k => lines.push('  + ' + k)); lines.push(''); }
    lines.push('═══════════════════════════════════════════');
    lines.push('  jobbachao.com — AI Career Defense');
    lines.push('═══════════════════════════════════════════');
    ...
    a.href = url;
    a.download = `weaponized-resume-${role.toLowerCase().replace(/\s+/g, '-')}.txt`;
