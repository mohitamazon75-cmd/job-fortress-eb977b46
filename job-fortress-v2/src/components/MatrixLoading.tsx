  ...
  const getWaitMessage = () => {
    ...
    if (elapsedSec >= 45) {
      return {
        ...
        subtitle: (() => {
          const tier = seniorityTier || 'PROFESSIONAL';
          if (tier === 'ENTRY' || tier === 'PROFESSIONAL') return 'Analyzing urgency — junior roles face fastest disruption…';
          if (tier === 'EXECUTIVE' || tier === 'SENIOR_LEADER') return 'Evaluating strategic positioning and leadership edge…';
          return 'Computing your personalized risk profile…';
        })(),
        showLeaveHint: false,
      };
    }
    return {
      ...
      subtitle: (() => {
        const tier = seniorityTier || 'PROFESSIONAL';
        if (tier === 'ENTRY' || tier === 'PROFESSIONAL') return 'Scanning entry-level displacement vectors…';
        if (tier === 'EXECUTIVE' || tier === 'SENIOR_LEADER') return 'Mapping organizational leverage signals…';
        return 'Profiling your career data…';
      })(),
      showLeaveHint: false,
    };
  };
