...
import { ArrowRight, Sparkles, Zap, Shield, Brain, TrendingUp, TrendingDown, Minus, Swords, Target, Search, Database, BarChart3, Globe, Lock, Bot, Loader2 } from 'lucide-react';
...
function IntelligenceProfile({ report, scanId }: { report: ScanReport; scanId?: string }) {
        ...
        <div className="flex items-center gap-3 mt-3 pt-2 border-t border-border">
          ...
          {survivability && (
            <span
              ...
              title="Estimated percentile based on your role's automation exposure vs. typical profiles in our model — not a live peer comparison"
              aria-label={`${survivability.peer_percentile_estimate} percentile — estimated based on automation exposure modelling, not live peer data`}
            >
              · {survivability.peer_percentile_estimate} est. percentile
            </span>
          )}
        </div>
      </motion.div>
...
function DossierCollapsible({ dossierText, streamComplete, streamError, markdownComponents }: {
  dossierText: string; streamComplete: boolean; streamError: string; markdownComponents: any;
}) {
...
// ── Main Component ──
export default function AIDossierReveal({ report, onComplete, scanId }: AIDossierRevealProps) {
    ...
    <div className="fixed inset-0 z-[200] bg-background overflow-hidden flex flex-col">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto">
        <div className="max-w-lg mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
          ...
          >
            <IntelligenceProfile report={report} scanId={scanId} />
          </motion.div>

          {/* ═══ AI DOSSIER — collapsible section ═══ */}
          {(dossierText || !streamComplete) && (
